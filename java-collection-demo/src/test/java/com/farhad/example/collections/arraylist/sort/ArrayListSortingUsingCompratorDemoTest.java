...
import com.farhad.example.collections.arraylist.sort.ArrayListSortingUsingCompratorDemo.Person;
import com.farhad.example.collections.arraylist.sort.ArrayListSortingUsingCompratorDemo.PersonSorter;
...
@Slf4j
public class ArrayListSortingUsingCompratorDemoTest {
    ...
    @Test
    public void sortPapersByNameTest() {
        log.info("{} Befor sort", persons.stream().map(person -> person.getName() ).collect(Collectors.toList()));
        List<Person> sortedPersons = personSorter.sortByName();
        log.info("{} After sort by age ascending", sortedPersons.stream().map(person -> person.getName() ).collect(Collectors.toList()));
    }
